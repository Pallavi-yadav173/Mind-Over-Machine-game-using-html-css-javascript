...
const LEFT_WALL  = 0
const RIGHT_WALL = canvas.width

// ═══════════════════════════════════════════════════════════════
//  AUDIO SETUP
//  ▸ land.mp3  — BGM, loops continuously across all 3 waves
//  ▸ sword.mp3 — SFX for every attack: player AND enemies
// ═══════════════════════════════════════════════════════════════

// BGM — loops forever until the game ends
const bgm = new Audio('assets/land.mp3')
bgm.loop   = true    // ← loops so it covers all waves regardless of track length
bgm.volume = 0.5
...
window.addEventListener('click',   ensureBGM, { once: true })

// Sword SFX pool — used by both player and enemies
const swordPool = Array.from({ length: 6 }, () => {
    const a = new Audio('assets/sword.mp3')
    a.volume = 0.55
    return a
})
let swordIdx = 0
...
function playSword() {
    if (muted) return
    const snd = swordPool[swordIdx++ % swordPool.length]
    snd.currentTime = 0
    snd.play().catch(() => {})
}
...
muteBtn.style.cssText = `
    ...
    background:rgba(10,7,0,0.85); border:1px solid #b8860b;
    color:#f5c518; font-size:18px; width:40px; height:40px;
...
function clampFighter(fighter, spriteWidth = 60) {
    if (fighter.position.x < LEFT_WALL) {
        fighter.position.x = LEFT_WALL
        fighter.velocity.x = 0
    }
    if (fighter.position.x + spriteWidth > RIGHT_WALL) {
        fighter.position.x = RIGHT_WALL - spriteWidth
        fighter.velocity.x = 0
    }
    ...
    if (fighter.position.y < -200) {
        fighter.position.y = -200
        fighter.velocity.y = 0
    }
}
...
function startWave2() {
    ...
    // BGM keeps playing — no audio changes between waves
    waveTransitioning = true
    setTimeout(() => {
        showLevelMessage('⚔️ Wave 2 Beginning...<br>Get Ready!', () => {
...
function updateEnemyHealthBar() {
    if (wave3Enemies.length === 0) return
...
function handleCollision(currentEnemy) {
    ...
    if (!currentEnemy.dead && currentEnemy.isAttacking && currentEnemy.framesCurrent === currentEnemy.framesMax - 1)
        currentEnemy.isAttacking = false
}
...
function animate() {
    ...
    if (wave === 1 || wave === 2) {
        ...
        if (!gameOver) {
            ...
            } else if (currentEnemy.health <= 0 && !currentEnemy.dead) {
                currentEnemy.dead = true
                gameOver = true

                if (wave === 1) {
                    setTimeout(() => {
                        showLevelMessage('⚔️ Wave 1 Complete!<br>Get Ready for Wave 2!', startWave2)
                    }, 1200)
                } else {
                    setTimeout(() => {
    ...
    if (wave === 3) {
        ...
        clampFighter(player)
        wave3Enemies.forEach(e => clampFighter(e))
        ...
        if (!gameOver) {
            if (player.health <= 0) {
                ...
                gameOver = true
                clearTimeout(timerId)
                bgm.pause()  // ← stop BGM on final victory

                localStorage.setItem('level3Complete', 'true')

                const displayText = document.querySelector('#displayText')
                displayText.innerHTML = `
                    🏆 YOU WIN!<br>All Enemies Defeated!
                    <button class="btn-aurum btn-levels" onclick="window.location.href='/end'">
                        Exit
                    </button>
                `
                displayText.style.display = 'flex'
